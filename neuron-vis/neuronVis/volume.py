...
class FNTCube:
    ...
    def getVolumeFromIndex(self, x, y, z):
        # ---------------------------------------------------------
        # 1. CALCULATE BLOCK INDICES
        # We must divide the pixel coordinate by the block size
        # ---------------------------------------------------------
        idx_x = int(x // self.cubesize[0])
        idx_y = int(y // self.cubesize[1])
        idx_z = int(z // self.cubesize[2]) # This is your "/100/" folder
        
        # ---------------------------------------------------------
        # 2. CONSTRUCT URL
        # Pattern: .../cube/{Z_INDEX}/{X}_{Y}_{Z}.tif
        # ---------------------------------------------------------
        ...
        # DEBUG: Uncomment this to verify it matches your browser link
