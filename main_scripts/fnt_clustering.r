...
# Pivot to matrix (equivalent to Python's pivot)
raw_matrix <- raw_df %>% 
  select(i, j, score) %>%
  pivot_wider(names_from = j, values_from = score, values_fill = 0) %>%
  column_to_rownames("i") %>%
  as.matrix()

# Ensure symmetric (in case pivot didn't create square matrix)
all_names <- union(rownames(raw_matrix), colnames(raw_matrix))
raw_matrix <- raw_matrix[all_names, all_names, drop = FALSE]
...
# ==========================================
# Allow user override
k_final <- as.integer(readline(prompt = paste("Enter K (Press Enter for", best_k, "): ")))
