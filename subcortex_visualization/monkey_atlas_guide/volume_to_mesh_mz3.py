...
def get_unique_indices(nifti_file):
    """Get list of unique non-zero indices in the atlas."""
    img = nib.load(nifti_file)
    data = img.get_fdata()
    unique = np.unique(data)
    # Return only non-zero indices (exclude background)
    return sorted([int(x) for x in unique if x > 0])
...
def run_niimath(nifti_file, idx, outpath=None):
    ...
    if result.returncode != 0:
        print(f"  Warning: Region {idx} failed (probably empty)")
        return result.returncode
    else:
        print(f"  ✓ Saved: {outname}.mz3")
        return 0
...
if __name__ == "__main__":
    ...
    # Get indices that actually exist in the file
    existing_indices = get_unique_indices(nifti_file)
    
    # Filter by min/max if specified
    if index_min is not None:
        existing_indices = [i for i in existing_indices if i >= index_min]
    if index_max is not None:
        existing_indices = [i for i in existing_indices if i <= index_max]
    
    print(f"Found {len(existing_indices)} regions to process")
    print(f"Index range: {existing_indices[0]} to {existing_indices[-1]}")
    if len(existing_indices) <= 20:
        print(f"Indices: {existing_indices}")

    region_count = 1
    failed_indices = []

    for idx in existing_indices:
        result = run_niimath(nifti_file, idx, outpath)
        if result != 0:  # niimath failed
    ...
    # Summary
    total_processed = len(existing_indices) - len(failed_indices)
    print(f"\n{'='*50}")
    print(f"Summary: {total_processed}/{len(existing_indices)} regions converted successfully")
    if failed_indices:
        print(f"Failed indices ({len(failed_indices)}): {failed_indices[:10]}...")
    print(f"{'='*50}\n")
